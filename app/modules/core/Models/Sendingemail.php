...
class Core_Model_Sendingemail
{
  ...
  public function sendMailContact($data)
  {
    ...
    $this->email->getMail()->addBCC("desarrollo8@omegawebsystems.com", "Contáctanos FENDESA");
    $content = $this->_view->getRoutPHP('/../app/modules/core/Views/templatesemail/mailContact.php');
    $this->email->getMail()->Subject = 'Contáctanos FENDESA';
    $this->email->getMail()->msgHTML($content);
    $this->email->getMail()->AltBody = $content;
   //$this->email->getMail()->addBCC($informacion->info_pagina_correo_oculto, "Contáctanos FENDESA");
    if ($this->email->sed() == true) {
      return 1;
    } else {
      return 2;
