...
class Page_Model_Template_Template
{
	...
	public function getContentseccion($seccion){
		...
		$contenidos = [];
		$rescontenidos = $contenidoModel->getList("contenido_estado='1' AND contenido_seccion = '$seccion' AND contenido_padre = '0' AND contenido_seccion != '2'   ","orden ASC");
	...
	public function getContentseccionHome($seccion){
		$contenidoModel = new Page_Model_DbTable_Contenido();
		$contenidos = [];
		...
		$this->_view->contenidos = $contenidos;
		return $this->_view->getRoutPHP("modules/page/Views/template/contenedorHome.php");
	}
